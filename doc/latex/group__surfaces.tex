...
Built-\/in geometries are quite simple but are sufficient for testing of code and checking the toolchain. There are also grid geometries which are not intended for calculation but are suitable for visualizing the potential field around surfaces after a problem has been solved. 
...
Generate a regular grid in plane $z=0$ which can be transformed and used for visualization. Nodes are interpolated between $(s_{\min},t_{\min},0)$ and $(s_{\max},t_{\max},0)$
